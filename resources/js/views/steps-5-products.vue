    ...
    <div class="mx-auto w-full lg:w-2/4 max-w-screen-md">
        <div class="vx-row">
            <div class="vx-col w-full sm:w-full md:w-full lg:w-full xl:w-2/3 text-left">
                <router-link :to="{name: 'keto-quiz-step4'}">
        ...
        <div class="vx-row">
            <div class="vx-col w-full mb-base">
                <vx-card>
                    <vs-progress class="block" :percent="60" color="primary"></vs-progress>
                    ...
                    <select-button v-for="item in this.products" :key="item" @click="selectProduct(item)" :isIcon="true" :query="item">
                        {{ capitalize(item) }}
                    </select-button>
...
export default {
  data() {
      return {
        products: [], //[ 'onions', 'mushrooms', 'eggs', 'nuts', 'cheese', 'butter', 'milk', 'avocado', 'seafood', 'olives', 'capers', 'coconut', 'goat-cheese',],
        selected_products: [],
        errMsg: '',
        limit: 5,
      }
  ...
  beforeRouteEnter(to, from, next) {
      if(from.name != 'keto-quiz-step4' && from.name != 'keto-quiz-step6') {
          next({
              name: 'keto-main',
          })
      } else {
  ...
  watch: {
      selected_products: function(after, before) {
          if(after.length > 0 && this.errMsg == 'Please select an answer') {
            this.errMsg = ''
            return
          } else if(after.length >= this.limit && this.errMsg == 'Please select more products in order to get a complete and diversified meal plan.') {
            this.errMsg = ''
            return
          }
      }
  ...
  methods: {
      ...
      selectProduct(product) {
        for(var i=0; i<this.selected_products.length; i++) {
            if(this.selected_products[i] == product) {
      ...
      nextpage() {
        if(!this.selected_products.length) {
            ...
            return
        } else if(this.selected_products.length < this.limit) {
            this.errMsg = "Please select more products in order to get a complete and diversified meal plan."
            return
        }
        this.$store.dispatch("keto/set_meat_products", this.selected_products).then(() => {
            this.$router.push({name: 'keto-quiz-step6'})
        })    
      },
  }
}
