...
if (mix.inProduction()) {
    ...
    if( process.env.NPM_START_COMMAND === "npm run hot" ) {
        mix.options({
            hmrOptions: {
                host: 'fitnesi-app.local',  // site's host name
                port: 8080,
            }
        });
        mix.webpackConfig({
            ...
            output: {
                //pathInfo: false,
                publicPath: 'http://fitnesi-app.local:8080/',
                chunkFilename: 'js/chunks/[name].[chunkhash].js',
            },
            devServer: {
                inline:true,
                ...
                public: 'http://fitnesi-app.local:8080',
        ...
        mix.browserSync( {
            open: false, proxy: 'nginx',watch: false
        });
    }
}
